...
public class ServiceImplGenerator extends Generator {
    ...
    public ServiceImplGenerator(Boolean isOverWrite, GenerateInfo generateInfo, TemplateFileNameEnum tpFileName) {
    ...
    public VirtualFile generate() throws IOException, TemplateException {
        ...
        //判断包是否存在
        File packagePath = new File(generateInfo.getGenerateJava().getServiceImplPackagePath());
        if (!packagePath.exists()) {
            FileUtils.forceMkdir(packagePath);
        }
        VirtualFile packageDir = VfsUtil.findFile(packagePath.toPath(), true);

        VirtualFile virtualFile = packageDir.findChild(generateInfo.getGenerateJava().getServiceImplFileName());
        if (Objects.isNull(virtualFile)) {
            virtualFile = packageDir.createChildData(generateInfo.getProject(), generateInfo.getGenerateJava().getServiceImplFileName());
        } else {
            if (!isOverWrite) {
                return null;
            }
